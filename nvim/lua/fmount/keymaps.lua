...
-- Diagnostics
keymap('n', '<leader>dd', vim.diagnostic.open_float, { desc = 'Open diagnostic' })
...
-- Windows Resizing
keymap('n', '<leader><', ':vertical resize +5<CR>', { desc = 'Increase window width' })
keymap('n', '<leader>>', ':vertical resize -5<CR>', { desc = 'Decrease window width' })
...
-- Font resize (GUI only)
vim.api.nvim_create_autocmd("UIEnter", {
  once = true,
  callback = function()
    if not vim.g.GuiLoaded then return end
    local fontsize = 12
...
-- VIM-NOTES mappings
keymap('n', '<Leader>ni', '<Plug>(note-new-cbox-inline)', { remap = true, desc = 'New inline checkbox' })
keymap('i', '<Leader>ni', '<Plug>(note-new-cbox-inline)', { remap = true, desc = 'New inline checkbox' })
keymap('n', '<Leader>no', '<Plug>(note-new-cbox-below)', { remap = true, desc = 'New checkbox below' })
keymap('i', '<Leader>no', '<Plug>(note-new-cbox-below)', { remap = true, desc = 'New checkbox below' })
keymap('n', '<Leader>nO', '<Plug>(note-new-cbox-above)', { remap = true, desc = 'New checkbox above' })
keymap('i', '<Leader>nO', '<Plug>(note-new-cbox-above)', { remap = true, desc = 'New checkbox above' })
